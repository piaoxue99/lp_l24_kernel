...
struct switch_dev *local_sdev = NULL;
...
static ssize_t hdmi_common_wta_external_block(struct device *dev, struct device_attribute *attr,
...
static int __devinit hdmi_tx_probe(struct platform_device *pdev)
{
	...
	local_sdev = &hdmi_ctrl->sdev;
	if(!local_sdev) {
		DEV_ERR("%s: FAILED: hdmi_ctrl->sdev not found\n", __func__);
		return -ENODEV;
	}
