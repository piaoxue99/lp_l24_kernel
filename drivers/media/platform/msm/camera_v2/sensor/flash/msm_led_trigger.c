...
#if defined(CONFIG_MACH_MSM8974_DZNY_DCM)
static int32_t msm_led_get_batt_temp(struct msm_led_flash_ctrl_t *fctrl)
{
	...
	vadc_dev = qpnp_get_vadc(&fctrl->pdev->dev, "flash");
	rc = qpnp_vadc_read(vadc_dev, LR_MUX1_BATT_THERM, &results);

	pr_info("%s : current batt_temp = %d, rc = %d\n", __func__, (int)results.physical, rc);
...
#if defined(CONFIG_MACH_MSM8974_DZNY_DCM)
		batt_temp = msm_led_get_batt_temp(fctrl);
		if(batt_temp <= -100) {
			pr_info("%s %d low batt_temp, change torch_current\n", __func__, __LINE__);
			cfg->torch_current = 50;
		}
#endif
		if (fctrl->torch_trigger) {
...
#if defined(CONFIG_MACH_MSM8974_DZNY_DCM)
		if(batt_temp <= -100) {
			pr_info("%s %d low batt_temp, change flash_current\n", __func__, __LINE__);
			if (fctrl->torch_trigger)
				led_trigger_event(fctrl->torch_trigger, 0);
			usleep(1 * 1000);
			cfg->torch_current = 100;
			if (fctrl->torch_trigger) {
				max_curr_l = fctrl->torch_max_current;
				if (cfg->torch_current > 0 &&
						cfg->torch_current <= max_curr_l) {
