...
#define DOZE_RECALIBRATION_INTERVAL     (ts->f01.dsc.control_base + 5)
...
#define LPWG_PARTIAL_REG			(ts->f51.dsc.control_base+48)//F51_CUSTOM_CTRL14, Partial LPWG 0x485, 0x046d, 0x458
	...
	case LPWG_DOUBLE_TAP:                         /* Only TCI-1 */
		...
		if (!(strncmp(ts->fw_info.fw_product_id, "PLG446", 6))
			...
			if (lpwg_by_lcd_notifier) {
				TOUCH_INFO_MSG("Normal LPWG ON command will be work in LCD_Notifier\n");
			} else {
				TOUCH_INFO_MSG("Partial LPWG ON command\n");
...
static ssize_t show_lpwg_doze_interval(struct i2c_client *client, char *buf)
{
	int ret = 0;
	struct synaptics_ts_data *ts = (struct synaptics_ts_data *)get_touch_handle(client);

	if (strncmp(ts->fw_info.fw_image_product_id, "PLG503", 6)) {
		TOUCH_INFO_MSG("Not support :%s\n",ts->fw_info.fw_image_product_id);
		return ret;
	}

	mutex_lock(&ts->pdata->thread_lock);
	ret += snprintf(buf+ret, PAGE_SIZE-ret, "current_mode [%d]\n", current_mode);
	mutex_unlock(&ts->pdata->thread_lock);

	TOUCH_INFO_MSG("current_mode [%d]\n", current_mode);

	return ret;
}

static ssize_t store_lpwg_doze_interval(struct i2c_client *client,
		const char *buf, size_t count)
{
	int value = 0;
	u8 buffer[2] ={0xFF,0x63};
	struct synaptics_ts_data *ts = (struct synaptics_ts_data *)get_touch_handle(client);
	...
	mutex_lock(&ts->pdata->thread_lock);
	current_mode = value;
	synaptics_ts_page_data_write(client, COMMON_PAGE, DOZE_RECALIBRATION_INTERVAL, 1,
		(current_mode) ? &buffer[0] : &buffer[1]);
	TOUCH_INFO_MSG("Doze Recalibration Interval set to %s\n",
		(current_mode) ? "25.6sec (0xFF)" : "10.0sec(0x63)");
	mutex_unlock(&ts->pdata->thread_lock);
...
static LGE_TOUCH_ATTR(lpwg_doze_interval, S_IRUGO | S_IWUSR, show_lpwg_doze_interval, store_lpwg_doze_interval);
...
static struct attribute *synaptics_ts_attribute_list[] = {
	...
	&lge_touch_attr_lpwg_doze_interval.attr,
	NULL,
};
		...
		u8 code, u32 value, u32 *ret)
{
	...
	case IC_CTRL_BASELINE_REBASE:
		TOUCH_INFO_MSG("Send IC Rebase command... !!\n");
		DO_SAFE(synaptics_ts_page_data_write_byte(client,
	...
	case IC_CTRL_RESET:
		ts->is_init = 0;
		TOUCH_INFO_MSG("Send IC Reset command... !!\n");
		...
		u32 code, u32 value, struct point *data)
{
	...
	case LPWG_INCELL_LPWG_ON:
		...
		if(current_mode) {
			u8 buffer = 0xFF;
			TOUCH_DEBUG(DEBUG_BASE_INFO, "current mode [%d]\n", current_mode);
			synaptics_ts_page_data_write(client, COMMON_PAGE, DOZE_RECALIBRATION_INTERVAL, 1, &buffer);
		}
