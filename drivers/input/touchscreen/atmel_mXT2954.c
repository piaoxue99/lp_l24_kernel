...
#include "atmel_mXT2954.h"
#include "atmel_mXT2954_patch.h"

//self-test fix S
static u8 self_test_status_alltest = 0;
static u8 self_test_status_avdd_power[4]={0,0,0,0};
static u8 self_test_status_signal_limit[4]={0,0,0,0};
static u8 self_test_status_pin_fault[4]={0,0,0,0};
//self-test fix E
...
//		}
		...
		if ((status & MXT_T100_STATUS_MASK) == MXT_T100_PRESS)
			TOUCH_INFO_MSG("%d touch_pressed <%d> : x[%3d] y[%3d] P[%3d] WM[%3d] Wm[%3d] \n",
			id+1, id, x, y, amplitude, data->ts_data.curr_data[id].touch_major, data->ts_data.curr_data[id].touch_minor);
...
static irqreturn_t mxt_process_messages_t44(struct mxt_data *data)
{
	...
	ret = __mxt_read_reg(data->client, data->T44_address, data->T5_msg_size + 1, data->msg_buf);
	if (ret) {
	...
	if (count == 0) {
		/* Zero message_mxt_read_reg(data->client, data->T44_address, data->T5_msg_size  is too much occured.
...
static irqreturn_t mxt_interrupt(int irq, void *dev_id)
{
	...
	if (data->in_bootloader) {
		TOUCH_ERR_MSG("bootloader state\n");
	...
	if (!data->object_table) {
		TOUCH_ERR_MSG("object table is not set\n");
		return IRQ_HANDLED;
	}
...
static int mxt_t25_command(struct mxt_data *data, u8 value, bool wait)
{
	...
	if(value ==0xFE)
	{
	TOUCH_INFO_MSG("Dwell time is 200us for pin fault test.\n");
	reg = data->T25_address + 10 ; // dwell time
	ret = mxt_write_reg(data->client, reg, 200);
	}
...
static void mxt_regulator_enable(struct mxt_data *data)
{
	...
	error = regulator_enable(data->vdd_ana);
		TOUCH_INFO_MSG("vdd_ana regulator enable !!\n");
	if (error < 0) {
	...
	error = regulator_enable(data->vcc_i2c);
		TOUCH_INFO_MSG("vcc_i2c regulator enable !!\n");
	if (error < 0) {
	...
	error = regulator_enable(data->vcc_dig);
		TOUCH_INFO_MSG("vcc_dig regulator enable !!\n");
	if (error < 0) {
...
static void mxt_probe_regulators(struct mxt_data *data)
{
	...
	if (IS_ERR(data->vdd_ana)) {
		...
		dev_err(dev, "Error %d getting vdd_ana regulator\n", error);
		goto fail_release;
	}
	...
	if (IS_ERR(data->vcc_i2c)) {
		...
		dev_err(dev, "Error %d getting vcc_i2c regulator\n", error);
		goto fail_release3;
	}
	...
	if (IS_ERR(data->vcc_dig)) {
		...
		dev_err(dev, "Error %d getting vcc_dig regulator\n", error);
		goto fail_release2;
	}

	error = regulator_set_voltage(data->vdd_ana, 3300000, 3300000);
	if (error < 0) {
		dev_err(dev, "Error %d cannot control vdd_ana regulator\n", error);
		goto fail;
	}

	error = regulator_set_voltage(data->vcc_dig, 3300000, 3300000);
	if (error < 0) {
		dev_err(dev, "Error %d cannot control vcc_dig regulator\n", error);
		goto fail_release2;
	}
...
static void copy_array(u8* dest, u8* src)
{
	dest[0] = src[0];
	dest[1] = src[1];
	dest[2] = src[2];
...
static ssize_t mxt_selftest(struct mxt_data *data, char *buf, int len)
{
	...
	msleep(300);
	self_test_status_alltest = data->self_test_status[0];

	if(data->self_test_status[0]!=0x12)
	{
	TOUCH_INFO_MSG("Skip indivisual item.");
	copy_array(self_test_status_avdd_power, data->self_test_status);
	copy_array(self_test_status_signal_limit, data->self_test_status);
	copy_array(self_test_status_pin_fault, data->self_test_status);
	}
	else
	{
	TOUCH_INFO_MSG("Fail in pin fault test. Re-test indivisual item.");
	copy_array(self_test_status_pin_fault, data->self_test_status);

	TOUCH_INFO_MSG("Start AVdd power Test.");
	selftest_enable = true;
	selftest_show = true;
	...
	TOUCH_INFO_MSG("Start Signal Limit Test.");
	selftest_enable = true;
	selftest_show = true;
	...
	if (self_test_status_alltest == 0) {
		ret += snprintf(buf + ret, PAGE_SIZE - ret,  "Need more time. Try Again.\n");
		return ret;
	}

	if (self_test_status_alltest == 0xFD) {
		ret += snprintf(buf + ret, PAGE_SIZE - ret, "Invalid Test Code. Try Again.");
	} else if (self_test_status_alltest == 0xFC) {
		ret += snprintf(buf + ret, PAGE_SIZE - ret, "The test could not be completed due to an unrelated fault. Try again.");
	} else {
		ret += snprintf(buf + ret, PAGE_SIZE - ret, "All Test Result: %s", (self_test_status_alltest==0xFE) ? "Pass\n" : "Fail\n");
		ret += snprintf(buf + ret, PAGE_SIZE - ret, "AVdd power Test Result: %s", (self_test_status_avdd_power[0] != 0x01) ? "Pass\n" : "Fail\n");

		ret += snprintf(buf + ret, PAGE_SIZE - ret, "Pin Fault Test Result: %s", (self_test_status_pin_fault[0] != 0x12) ? "Pass\n" : "Fail\n");
		if (self_test_status_pin_fault[0] == 0x12)
			ret += snprintf(buf+ret, PAGE_SIZE - ret, "# Fail # seq_num(%u) x_pin(%u) y_pin(%u)\n",
									self_test_status_pin_fault[1], self_test_status_pin_fault[2], self_test_status_pin_fault[3]);

		ret += snprintf(buf + ret, PAGE_SIZE - ret, "Signal Limit Test: %s", (self_test_status_signal_limit[0] != 0x17) ? "Pass\n" : "Fail\n");
		if (self_test_status_signal_limit[0] == 0x17)
			ret += snprintf(buf+ret, PAGE_SIZE - ret, "# Fail # type_num(%u) type_instance(%u)\n", self_test_status_signal_limit[1], self_test_status_signal_limit[2]);
	}
...
//	mxt_get_cap_diff(data);
	...
	if (self_test_status_avdd_power[0] == 0x01)
		chstatus_result = 0;
	...
	} else if (self_test_status_alltest == 0xFD) {
...
#if 0 // For Debug
	TOUCH_ERR_MSG("mxt_knock_on_enable : %d / lpwg_mode : %d / mfts_enable : %d\n", data->mxt_knock_on_enable, data->lpwg_mode, data->mfts_enable);
	TOUCH_ERR_MSG("!lpwg_mode && !mfts_enable : %d\n", (!data->lpwg_mode && !data->mfts_enable));
	TOUCH_ERR_MSG("!mxt_knock_on_enable && !mfts_enable : %d\n", (!data->mxt_knock_on_enable && !data->mfts_enable));
#endif
...
static int mxt_parse_dt(struct device *dev, struct mxt_platform_data *pdata)
{
	...
	pdata->gpio_sda = of_get_named_gpio_flags(node, "atmel,sda-gpio", 0, NULL);
	if (pdata->gpio_sda)
		TOUCH_INFO_MSG("DT : gpio_sda = %lu\n", pdata->gpio_sda);
	else
		TOUCH_INFO_MSG("DT get gpio_sda error \n");

	pdata->gpio_scl = of_get_named_gpio_flags(node, "atmel,scl-gpio", 0, NULL);
	if (pdata->gpio_scl)
		TOUCH_INFO_MSG("DT : gpio_scl = %lu\n", pdata->gpio_scl);
	else
		TOUCH_INFO_MSG("DT get gpio_scl error \n");
...
static int mxt_read_t100_config(struct mxt_data *data)
{
	...
	error = __mxt_read_reg(client, object->start_address + MXT_T100_XRANGE, sizeof(range_x), &range_x);
	if (error)
		return error;

	le16_to_cpus(range_x);

	error = __mxt_read_reg(client, object->start_address + MXT_T100_YRANGE, sizeof(range_y), &range_y);
	if (error)
		return error;

	le16_to_cpus(range_y);

	error =  __mxt_read_reg(client, object->start_address + MXT_T100_CFG1, 1, &cfg);
	if (error)
		return error;

	error =  __mxt_read_reg(client, object->start_address + MXT_T100_TCHAUX, 1, &tchaux);
	if (error)
		return error;
	...
	TOUCH_INFO_MSG("T100 Touchscreen size X : %u / Y : %u\n", data->max_x, data->max_y);
...
static int mxt_flash_fw_on_probe(struct mxt_fw_info *fw_info)
{
	...
	TOUCH_INFO_MSG("Binary Version [IC:%u.%u.%02X] [FW:%u.%u.%02X]\n",
		data->info->version >> 4, data->info->version & 0xF, data->info->build,
		fw_info->bin_ver >> 4, fw_info->bin_ver & 0xF, fw_info->build_ver);
...
static int __devinit mxt_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	error = request_threaded_irq(data->irq, NULL, mxt_interrupt, IRQF_TRIGGER_FALLING | IRQF_ONESHOT | IRQF_NO_SUSPEND, client->name, data);
	if (error) {
